...
def bocpd_deriv(theta_h, theta_m, X, hazard_f, model_f):
  ...
  for t in range(T):
    ...
    R[1:t+1, t+1] = R[:t,t]*model_f.predprobs*(1 - H) # t x 1. [P]
    for ii in range(hazard_f.num_hazard_params):
      dR_h[1:t+1, t+1, ii] = model_f.predprobs*(dR_h[:t, t, ii]*(1 - H)-R[:t, t]*dH[:,ii])

    for ii in range(num_model_params):
      dR_m[1:t+1, t+1, ii] = (1 - H)*(dR_m[:t,t,ii]*model_f.predprobs+ R[:t,t]* model_f.dpredprobs[:,ii])
    ...
    R[0, t+1] = np.sum(R[:t, t] * model_f.predprobs * H) # 1 x 1. [P]
    for ii in range(hazard_f.num_hazard_params):
      dR_h[0, t+1, ii] = np.sum(model_f.predprobs * (dR_h[:t,t,ii]*H + R[:t,t] * dH[:, ii]))
    for ii in range(model_f.num_model_params):
      dR_m[0, t+1, ii] = np.sum(H * (dR_m[:t, t, ii] * model_f.predprobs + R[:t,t] * model_f.dpredprobs[:, ii]))
    ...
    model_f.update(X[t, :], needDerivatives=True)
  ...
  dnlml_h = np.zeros((hazard_f.num_hazard_params, 1))
  dnlml_m = np.zeros((model_f.num_model_params, 1))
  for ii in range(hazard_f.num_hazard_params):
