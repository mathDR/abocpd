...
class gaussian1D():
    ...
    def update(self,theta_prior,xt,needDerivatives=False):
        ...
        mus_new    = [theta_prior[0]].extend((kappas*mus + xt)/(kappas + 1))
        kappas_new = [theta_prior[1], kappas+1] # N + 1 x 1. [points]
        # p(precision X|X) = Gamma(alphas, betas), alphas = shape, betas = inv scale
        # => E[precision X] = alpha / beta => E[var X] = beta / alpha.
        alphas_new = [theta_prior[2], alphas + 0.5] # N + 1 x 1. [points]
        # N + 1 x 1. [x^2]
        betas_new  = [theta_prior[3], betas + (kappas*(xt - mus)**2)/(2.0*(kappas+1))]
    ...
    def predict(self,xnew,needDerivatives=False):
      ...
      if not needDerivatives:
        self.pred = studentpdf(xnew, mus, predictive_variance, df) # N x 1. [P/x]
      else:
        self.pred, dtpdf = studentpdf(xnew, mus, predictive_variance, df, nargout=2) # N x 1. [P/x]
        dmu_dtheta    = np.transpose(self.dpost_params[0, :, :], axes=[3,2,1]) # N x 4
        dkappa_dtheta = np.transpose(self.dpost_params[1, :, :], axes=[3,2,1]) # N x 4
        dalpha_dtheta = np.transpose(self.dpost_params[2, :, :], axes=[3,2,1]) # N x 4
        dbeta_dtheta  = np.transpose(self.dpost_params[3, :, :], axes=[3,2,1]) # N x 4
